...
#define RKNN_ERR_INCOMPATILE_OPTIMIZATION_LEVEL_VERSION \
    -12                                      /* This RKNN model set optimization level, but not compatible with current driver. */
...
typedef uint64_t rknn_context;
    ...
    */
    typedef enum _rknn_query_cmd
    {
        ...
        RKNN_QUERY_CURRENT_NATIVE_INPUT_ATTR =
            16, /* query the current native shape of rknn input tensor, only valid for dynamic rknn model*/
        RKNN_QUERY_CURRENT_NATIVE_OUTPUT_ATTR =
            17, /* query the current native shape of rknn output tensor, only valid for dynamic rknn model*/
    ...
    */
    typedef enum _rknn_tensor_mem_flags
    {
        RKNN_TENSOR_MEMORY_FLAGS_ALLOC_INSIDE =
            1, /*Used to mark in rknn_destroy_mem() whether it is necessary to release the "mem" pointer itself.
        ...
        RKNN_TENSOR_MEMORY_FLAGS_FROM_FD =
            2, /*Used to mark in rknn_create_mem_from_fd() whether it is necessary to release the "mem" pointer itself.
        ...
        RKNN_TENSOR_MEMORY_FLAGS_FROM_PHYS =
            3, /*Used to mark in rknn_create_mem_from_phys() whether it is necessary to release the "mem" pointer itself.
    ...
    */
    ...
    /*  rknn_set_input_shapes
        ...
        set all the input tensor shapes. graph will run under current set of input shapes after rknn_set_input_shapes.(only valid for
